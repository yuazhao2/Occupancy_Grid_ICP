...
#使用generalized ICP (plane to plane)对给定两个点云进行配准，无visualisation,适合命令行批量处理
#使用方法：
#python ICP_generalized.py source.pcd target.pcd initialisation.txt result.txt
...
if __name__ == "__main__":   
    ...
    result_ICP =  o3d.pipelines.registration.registration_generalized_icp(
        ...
        o3d.pipelines.registration.TransformationEstimationForGeneralizedICP(), crit)
    print("Generalized ICP registration took %.3f sec.\n" % (time.time() - start))
    print(result_ICP)
